...
Global Dim Lng.s(8)
Lng(1) = "Specify time zone"
Lng(2) = "For example, for Moscow it is 3"
Lng(3) = "Error"
Lng(4) = "No access to server"
Lng(5) = "Set this time in the tray?"
Lng(6) = "Successful with "
Lng(7) = " in "
Lng(8) = " msec"

If UserIntLang = 1049
	Lng(1) = "Указать зону"
	Lng(2) = "Например для Москвы это 3"
	Lng(3) = "Ошибка"
	Lng(4) = "Нет доступа к серверу"
	Lng(5) = "Задать это время в трее?"
	Lng(6) = "Удачно с "
	Lng(7) = " за "
	Lng(8) = " мсек"
EndIf
...
If notauto
	If NotINI And Not Count
		Input$ = InputRequester(Lng(1), Lng(2), Str(GetTimeZoneOffset()))
		If Asc(Input$)
			addhour = Val(Input$)
		EndIf
	EndIf
...
If notauto = 2
	...
	If MessageRequester(Lng(5),
						FormatDate("%hh:%ii:%ss %dd.%mm.%yyyy",
						DateTime) + #CRLF$ + Lng(6) + #DQUOTE$ + Servers() + #DQUOTE$ + Lng(7) + Str(Timer) + Lng(8),
